...
const DeliveryPlanningPage: React.FC = () => {
  ...
  const handleLoadStores = async () => {
    try {
      ...
      message.success(
        `数据加载完成: 总数${
          (result.stores || []).length + (result.unlocatedStores || []).length
        }, ` +
    ...
    try {
      ...
      const assignedStoreIds = new Set(
        result.routes.flatMap((route) =>
          route.stops.map((stop) => stop.store.id)
        )
      );
      const unassignedStores = stores.filter(
        (store) => !assignedStoreIds.has(store.id)
      );
  ...
  const renderRouteDetail = (route: DeliveryRoute) => {
    // 计算实际总时间（行驶时间 + 停留时间）
    const totalStopDuration = route.stops.reduce(
                  ...
                  <div className="bg-green-50 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <div>
                          {Math.round(totalStopDuration / route.stops.length)}
                          分钟/店
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="mt-2 pt-2 border-t">
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div>
                              ...
                              <div>
                                {(stop.drivingInfo.distance / 1000).toFixed(1)}
                                公里
                              </div>
                            </div>
                            <div>
                              <div className="text-gray-500">行驶时间</div>
                              <div>
                                {Math.ceil(stop.drivingInfo.duration / 60)}分钟
                              </div>
                            </div>
                          </div>
                        </div>
            ...
            <div className="space-y-4">
              ...
              {loading && planningProgress.total > 0 && (
                <div className="mt-4">
                  ...
                  <Progress
                    percent={Math.round(
                      (planningProgress.current / planningProgress.total) * 100
                    )}
                    status="active"
                    format={(percent) =>
                      `${planningProgress.current}/${planningProgress.total} (${percent}%)`
                    }
                  />
                </div>
              )}
                          ...
                          },
                          {
                            ...
                            render: (_, record: Store) => {
                              ...
                              const distance =
                                Math.sqrt(
                                  Math.pow(
                                    record.location.latitude - 30.877369,
                                    2
                                  ) +
                                    Math.pow(
                                      record.location.longitude - 120.093902,
                                      2
                                    )
                                ) * 111; // 粗略计算公里数
